...
class NHP_Options_background extends NHP_Options{
	...
	*/
	function render(){
		...
		$css_options = array(
			'repeat'	 => array(
				''		    => __('Default', MTS_THEME_TEXTDOMAIN ),
				'no-repeat' => __('No Repeat', MTS_THEME_TEXTDOMAIN ),
				'repeat'	=> __('Repeat All', MTS_THEME_TEXTDOMAIN ),
				'repeat-x'  => __('Repeat Horizontally', MTS_THEME_TEXTDOMAIN ),
				'repeat-y'  => __('Repeat Vertically', MTS_THEME_TEXTDOMAIN ),
				//'inherit'   => 'Inherit',
			),
			'attachment' => array(
				''		  => __('Default', MTS_THEME_TEXTDOMAIN ),
				'fixed'   => __('Fixed', MTS_THEME_TEXTDOMAIN ),
				'scroll'  => __('Scroll', MTS_THEME_TEXTDOMAIN ),
				//'inherit' => 'Inherit',
			),
			'position'   => array(
				''			    => __('Default', MTS_THEME_TEXTDOMAIN ),
				'left top'	    => __('Left Top', MTS_THEME_TEXTDOMAIN ),
				'left center'   => __('Left center', MTS_THEME_TEXTDOMAIN ),
				'left bottom'   => __('Left Bottom', MTS_THEME_TEXTDOMAIN ),
				'center top'	=> __('Center Top', MTS_THEME_TEXTDOMAIN ),
				'center center' => __('Center Center', MTS_THEME_TEXTDOMAIN ),
				'center bottom' => __('Center Bottom', MTS_THEME_TEXTDOMAIN ),
				'right top'	 => __('Right Top', MTS_THEME_TEXTDOMAIN ),
				'right center'  => __('Right center', MTS_THEME_TEXTDOMAIN ),
				'right bottom'  => __('Right Bottom', MTS_THEME_TEXTDOMAIN ),
			),
			'size'	   => array(
				''		  => __('Default', MTS_THEME_TEXTDOMAIN ),
				//'inherit' => 'Inherit',
				'cover'   => __('Cover', MTS_THEME_TEXTDOMAIN ),
				'contain' => __('Contain', MTS_THEME_TEXTDOMAIN ),
			),
			'parallax'   => array(
				'0' => __('Off', MTS_THEME_TEXTDOMAIN ),
				'1' => __('On', MTS_THEME_TEXTDOMAIN ),
			),
		);
		...
		if ( $this->field['options']['color'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Background Color:', MTS_THEME_TEXTDOMAIN ).'</div>';
		...
		// Tabs
		echo '<div class="bg-opt-input-label">'.__( 'Background Image:', MTS_THEME_TEXTDOMAIN ).'</div>';
		echo '<fieldset class="green buttonset buttonset-tabs ui-buttonset">';
			if ( $this->field['options']['image_pattern'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_pattern" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Pattern', MTS_THEME_TEXTDOMAIN ).'</span></label>';
			}
			if ( $this->field['options']['image_upload'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_upload" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Upload', MTS_THEME_TEXTDOMAIN ).'</span></label>';
			}
			if ( $this->field['options']['gradient'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_gradient" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Gradient', MTS_THEME_TEXTDOMAIN ).'</span></label>';
			}
		...
		if ( $this->field['options']['image_upload'] !== false ) {
			...
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'_image_upload">'.__('Browse', MTS_THEME_TEXTDOMAIN ).'</a>';
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'_image_upload">'.__('Remove Upload', MTS_THEME_TEXTDOMAIN ).'</a>';
			echo '</fieldset>';
			...
			if ( $this->field['options']['repeat'] !== false ) {
				...
				echo '<label for="'.$this->field['id'].'_repeat" class="bg-opt-input-label">'.__( 'Background Repeat:', MTS_THEME_TEXTDOMAIN ).'</label>';
			...
			if ( $this->field['options']['attachment'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_attachment" class="bg-opt-input-label">'.__( 'Background Attachment:', MTS_THEME_TEXTDOMAIN ).'</label>';
			...
			if ( $this->field['options']['position'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_position" class="bg-opt-input-label">'.__( 'Background Position:', MTS_THEME_TEXTDOMAIN ).'</label>';
			...
			if ( $this->field['options']['size'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_size" class="bg-opt-input-label">'.__( 'Background Size:', MTS_THEME_TEXTDOMAIN ).'</label>';
		...
		if ( $this->field['options']['gradient'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">' . __('From:', MTS_THEME_TEXTDOMAIN ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_from" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][from]" value="'.$this->value['gradient']['from'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['from'].'" style="width:70px;"/>';
			echo '</div>';
			echo '<div class="color-gradient-step-wrapper">';
			echo '<div class="bg-opt-input-label">' . __('To:', MTS_THEME_TEXTDOMAIN ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_to" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][to]" value="'.$this->value['gradient']['to'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['to'].'" style="width:70px;"/>';
			echo '</div>';

			echo '<div class="color-gradient-direction-wrapper">';
			echo '<label for="'.$this->field['id'].'_gradient_direction" class="bg-opt-input-label">' . __('Direction:', MTS_THEME_TEXTDOMAIN ) . '</label>';
			echo '<select id="'.$this->field['id'].'_gradient_direction" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][direction]" class="color-gradient-direction">';
				echo '<option value="horizontal"' . selected( $this->value['gradient']['direction'], 'horizontal', false ) . '>' . __( 'Horizontal', MTS_THEME_TEXTDOMAIN ) . '</option>';
				echo '<option value="vertical"' . selected( $this->value['gradient']['direction'], 'vertical', false ) . '>' . __( 'Vertical', MTS_THEME_TEXTDOMAIN ) . '</option>';
			echo '</select>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
		...
		if ( $this->field['options']['parallax'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Parallax Effect:', MTS_THEME_TEXTDOMAIN ).'</div>';
