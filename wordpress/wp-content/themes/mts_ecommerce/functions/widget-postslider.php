...
class mts_post_slider_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__('MyThemeShop: Post Slider', MTS_THEME_TEXTDOMAIN ),
			array( 'description' => __( 'Display posts from multiple categories in an animated slider.', MTS_THEME_TEXTDOMAIN ) )
		);
	}

	public function form( $instance ) {
		$defaults = array(
			'title' => __( 'Featured Posts', MTS_THEME_TEXTDOMAIN ),
			'cat' => array(),
			'slides_num' => 3,
			'show_title' => 0,
			'slider_nav' => 'bullets',
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'slides_num' ); ?>"><?php _e( 'Number of Posts to show', MTS_THEME_TEXTDOMAIN ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'slider_nav' ); ?>"><?php _e( 'Navigaton type:', MTS_THEME_TEXTDOMAIN ); ?></label> 
			<select id="<?php echo $this->get_field_id( 'slider_nav' ); ?>" name="<?php echo $this->get_field_name( 'slider_nav' ); ?>" class="widefat">
				<option value="bullets"<?php selected( $instance['slider_nav'], 'bullets' ); ?>><?php _e( 'Bulleted', MTS_THEME_TEXTDOMAIN ); ?></option>
				<option value="arrows"<?php selected( $instance['slider_nav'], 'arrows' ); ?>><?php _e( 'Arrows', MTS_THEME_TEXTDOMAIN ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_title'); ?>">
				...
				<?php _e( 'Show Title', MTS_THEME_TEXTDOMAIN ); ?>
			</label>
		</p>
					...
					<div class="widget-slider widget-slider-<?php echo $slider_nav; ?>">
						...
						<div class="slide">
							<a href="<?php echo esc_url( get_the_permalink() ); ?>">
								...
								the_post_thumbnail( 'widgetfull', $attr );
								?>
								<?php if ( $show_title ) { ?>
