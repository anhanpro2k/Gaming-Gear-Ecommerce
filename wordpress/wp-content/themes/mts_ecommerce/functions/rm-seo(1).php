...
class MTS_RMU {
	...
	private function __construct( $config = array() ) {
		$config_defaults = array(
			...
			'link_label_install'      => __( 'Try it for FREE!', MTS_THEME_TEXTDOMAIN ),
			'link_label_activate'     => __( 'Click here to activate it.', MTS_THEME_TEXTDOMAIN ),

			// Add tab in MTS Theme Options.
			'add_theme_options_tab'   => true,
			'theme_options_notice'    => '<span style="display: block; line-height: 1.8; margin-bottom: 20px;">' . sprintf( __( '%1$s is a revolutionary SEO product that combines the features of many SEO tools. Its features give you the power of an entire SEO team with just a few clicks.', MTS_THEME_TEXTDOMAIN ), '<a href="https://mythemeshop.com/plugins/wordpress-seo-plugin/?utm_source=Theme+Options+Panel&utm_medium=Link+CPC&utm_content=Rank+Math+SEO+LP&utm_campaign=UserBackend" target="_blank">Rank Math SEO</a>' ) . ' @CTA' . '</span>',
			...
			'metabox_notice_install'  => sprintf( __( 'The new %1$s plugin will help you rank better in the search results.', MTS_THEME_TEXTDOMAIN ), '<a href="https://mythemeshop.com/plugins/wordpress-seo-plugin/?utm_source=SEO+Meta+Box&utm_medium=Link+CPC&utm_content=Rank+Math+SEO+LP&utm_campaign=UserBackend" target="_blank">Rank Math SEO</a>' ) . ' @CTA',

			/* Translators: %s is CTA, e.g. "Try it now!" */
			'metabox_notice_activate' => sprintf( __( 'The %1$s plugin is installed but not activated.', MTS_THEME_TEXTDOMAIN ), '<a href="https://mythemeshop.com/plugins/wordpress-seo-plugin/?utm_source=SEO+Meta+Box&utm_medium=Link+CPC&utm_content=Rank+Math+SEO+LP&utm_campaign=UserBackend" target="_blank">Rank Math SEO</a>' ) . ' @CTA',
	...
	public function dashboard_widget_output( $post, $callback_args ) {
		...
		<div class="rmu-dashboard-panel">
			<a class="rmu-dashboard-panel-close" id="rmu-dashboard-dismiss" href="http://schema.local/wp-admin/?rmu-dashboard=0""><?php _e( 'Dismiss', MTS_THEME_TEXTDOMAIN ); ?></a>
	...
	public function get_install_link( $class = '', $label = '' ) {
		...
		return '<a href="' . $install_link . '" class="thickbox ' . esc_attr( $class ) . '" title="' . esc_attr__( 'Rank Math SEO', MTS_THEME_TEXTDOMAIN ) . '">' . $label . '</a>';
	}
	...
	public function add_theme_options_seo_tab( $sections ) {
			...
			$sections[] = array(
					...
					'title'  => __( 'SEO', MTS_THEME_TEXTDOMAIN ),
					'desc'   => '<p class="description">' . __( 'Fix all your SEO issues with just a few clicks. Comply with all the latest Google\'s guidelines.', MTS_THEME_TEXTDOMAIN ) . '</p>',
					'fields' => array(
							'seo_message' => array(
									'id'    => 'mts_seo_message',
									'type'  => 'info',
									...
									'desc'  => strtr( $this->get_setting( 'theme_options_notice' ), array( '@CTA' => $this->get_install_or_activate_link() ) ) . $didyouknow . $this->get_install_or_activate_link( 'button button-primary', __( 'Install Now', MTS_THEME_TEXTDOMAIN ), __( 'Activate It', MTS_THEME_TEXTDOMAIN ) ) . $this->get_notice_css(),
							),
					),
			);
			return $sections;
